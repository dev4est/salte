...
function dbconfig()
{
	...
	if(!empty($dbname) && !empty($dbuser))
	{
		...
		if(empty($error))
		{
			mysqlrestore('simpla.sql', $mysqli);
...
function mysqlrestore($filename, $mysqli)
{
  $templine = '';
  $fp = fopen($filename, 'r');
  ...
  while(!feof($fp)) {
    ...
    if (substr($line, 0, 2) != '--' && $line != '') {
      ...
      if (substr(trim($line), -1, 1) == ';') {
        // Perform the query
          if ($mysqli->query($templine) === FALSE) {
              print('Error performing query \'<b>' . $templine . '</b>\': ' . $mysqli->error . '<br /><br />');
          }
